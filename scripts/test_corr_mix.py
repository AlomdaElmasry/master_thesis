...
corr_upsampled = torch.zeros((b, t - 1, 64, 64, 64, 64))
for i in range(16):
    for j in range(16):
        corr_pix_up = F.interpolate(corr[:, :, i, j], size=(64, 64)).view(b, t - 1, 1, 1, 64, 64)
        corr_upsampled[:, :, 4 * i:4 * i + 4:, 4 * j:4 * j + 4] = corr_pix_up.repeat(1, 1, 4, 4, 1, 1)

# Mix the features using corr as weight
c1 = feats[:, :, r_list].permute(0, 1, 3, 4, 2).reshape(b, c, h * w * (t - 1)).unsqueeze(3).permute(0, 1, 3, 2)
c2 = corr_upsampled.permute(0, 2, 3, 4, 5, 1).reshape(b, h * w, h * w * (t - 1)).permute(0, 2, 1).unsqueeze(1)
